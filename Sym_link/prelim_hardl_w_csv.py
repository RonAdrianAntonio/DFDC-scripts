...
def link():
    ...
    for x in metadata_tup:
        ...
        try:
            if x[2] == 'train':
                if x[1] == 'fake':
                    os.link(root_dir + "/" + x[0], train_fake_dir + "/" + x[0].replace('/', '_'))
                else:
                    os.link(root_dir + "/" + x[0], train_real_dir + "/" + x[0].replace('/', '_'))
            else:
                os.link(root_dir + "/" + x[0], test_dir + "/" + x[1] + "_" + x[0].replace('/', '_'))
        except FileNotFoundError:
            print("Did not link" + x[0])
